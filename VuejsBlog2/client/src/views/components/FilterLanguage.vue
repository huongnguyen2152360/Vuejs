<template>
  <div id="tagsLanguage">
    ...
    <el-main>
      ...
      <div v-else>
        <p class="no-general">Oops! Seems like there are no Language posts yet. Why don't you create one then?</p>
      </div>
      <!-- New Post Form -->
...
export default {
  ...
  methods: {
    ...
    getallPosts: function() {
      axios({
        method: 'get',
        url: 'https://vuejsblog-server.herokuapp.com/getAllPostsLanguage'
      }).then(rs => {
        let cmtIds = []
        let nocmtIds = []
        this.allPosts = rs.data
